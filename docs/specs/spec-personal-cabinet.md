...
**Чего нет:** Google OAuth, онбординг после первого входа, страница профиля с настройками и статистикой.

> ⚠️ **ВКонтакте OAuth** — перенесён в **Sprint 4**. Supabase не имеет нативного VK провайдера (требуется Netlify Function как OAuth-прокси). Реализуем после стабилизации Google OAuth на пилоте.
...
Добавить вход через Google как основной OAuth-способ. Оставить email/password как запасной вариант. Расположение кнопок: Google → разделитель «или» → форма email/password. После первого входа показать онбординг-экран (3 шага, < 1 минуты). Добавить «Профиль» — панель с базовыми данными и статистикой учителя.

_ВКонтакте OAuth — Sprint 4 (требует Netlify Function прокси, т.к. VK не поддерживается нативно в Supabase)._
