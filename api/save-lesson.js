/**
 * Vercel Edge Function: POST /api/save-lesson
 * Зеркало functions/api/save-lesson.js для параллельного деплоя на Vercel.
...
export default async function handler(req) {
  ...
  const lessonsBranch = process.env.GITHUB_BRANCH           || "main";

  if (!supabaseUrl || !serviceKey) return jsonResp({ error: "Auth сервис не настроен" }, 500);
  if (!githubToken)                return jsonResp({ error: "GITHUB_TOKEN не задан" }, 500);

  // ── JWT-верификация ─────────────────────────────────────────────────────────
  const authHeader = req.headers.get("Authorization");
  if (!authHeader) return jsonResp({ error: "Требуется авторизация" }, 401);
  ...
  try {
    ...
    if (!userResp.ok) return jsonResp({ error: "Недействительный токен" }, 401);
    verifiedUser = await userResp.json();
  } catch {
    return jsonResp({ error: "Ошибка проверки токена" }, 401);
  }

  const role = verifiedUser.user_metadata?.role;
  if (!role || !["teacher", "admin"].includes(role)) {
    return jsonResp({ error: "Доступ запрещён: только для учителей и администраторов" }, 403);
  }
  // ───────────────────────────────────────────────────────────────────────────

  let body;
  try { body = await req.json(); }
  catch { return jsonResp({ error: "Неверный JSON" }, 400); }

  const { lesson, meta } = body;
  if (!lesson || !meta?.subject || !meta?.grade || !meta?.topic) {
    return jsonResp({ error: "Обязательные поля: lesson, meta.subject, meta.grade, meta.topic" }, 400);
  }
  ...
  async function getFile(path) {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`GitHub GET ${path}: ${resp.status} — ${errMsg || resp.statusText}`);
    }
  ...
  try {
    ...
    return jsonResp({ ok: true, id, filename: filePath, total: indexArr.length });
  } catch (err) {
    return jsonResp({ error: "Ошибка сохранения: " + err.message }, 500);
  }
}
